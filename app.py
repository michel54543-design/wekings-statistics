...
def admin_wekings_events():
    denied = _admin_required()
    if denied: return denied
    return render_template_string(EVENTS_ADMIN_HTML, player_id=request.args.get("player_id", ""), events=None, analysis=None, result=None, error=None, days=3, cultures_count=0)
...
def admin_wekings_events_collect():
    ...
    try:
        ...
        # Готовим компактный список: только номер, дата/время и культура.
        ...
        for e in ordered:
            ...
            events.append({
                "when_short": local_dt.strftime("%H:%M %d.%m.%y"),
                "text_short": text_short,
            })
        events.reverse()
        cultures_count = len({e["text_short"] for e in events if e["text_short"]})
        return render_template_string(EVENTS_ADMIN_HTML, player_id=player_id, events=events, analysis=None, result=result, error=None, days=days, cultures_count=cultures_count)
    except Exception as exc:
        db.session.rollback()
        app.logger.exception("Garden events collection failed for player %s", player_id)
        return render_template_string(EVENTS_ADMIN_HTML, player_id=player_id, events=[], analysis=None, result=None, error=str(exc)[:1000], days=days, cultures_count=0)
...
def _build_delta_tops(before_at, current_at):
    """Calculate tops from one SQL join instead of loading two ORM snapshots.

    The old implementation materialized ~8k PlayerSnapshot objects twice and then
    walked them once for every metric. That made the first click on Top Today/
    Yesterday expensive and could hold the only web worker busy for a long time.
    """
    current = aliased(PlayerSnapshot)
    previous = aliased(PlayerSnapshot)
    metrics = YESTERDAY_TOP_METRICS
    columns = [current.player_id, current.nickname]
    for key, _label, _icon in metrics:
        columns.extend([getattr(current, key), getattr(previous, key)])
    ...
    rows = (
        ...
        .join(
            ...
            and_(
                previous.player_id == current.player_id,
                previous.batch_at == before_at,
            ),
        )
        .filter(current.batch_at == current_at)
        .all()
    )

    best = {key: None for key, _label, _icon in metrics}
    first_counts = {}
    first_total_gain = {}
    first_players = {}
    ...
    for row in rows:
        ...
        for i, (key, label, icon) in enumerate(metrics):
            current_value = row[2 + i * 2]
            previous_value = row[3 + i * 2]
            if current_value is None or previous_value is None:
                continue
            gain = int(current_value) - int(previous_value)
            if gain <= 0:
                continue
            old_best = best[key]
            if old_best is None or gain > old_best[0]:
                best[key] = (gain, player_id, nickname, label, icon)
    ...
    for key, label, icon in metrics:
        ...
        if not item:
            continue
        gain, player_id, nickname, label, icon = item
        tops.append({
            "metric": key,
            "label": label,
            "icon": icon,
